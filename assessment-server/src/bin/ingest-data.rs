use assessment_server::{Features, Record};
use clap::Parser;
...
struct Opts {
    ...
    #[arg(
        long,
        help = "If specified, will dump the data to mongodb.",
        default_value_t = false
    )]
    ingest: bool,
}

pub type IgnoreError<T = ()> =
...
pub fn parse_geojson<P: AsRef<std::path::Path>>(file: P) -> IgnoreError<Features> {
    ...
    info!(
        "Read to string successful. String length: {}",
        contents.len()
    );
...
pub async fn create_collection_if_not_exists(db: &Database, name: &str) -> IgnoreError {
    let all_collections = db.list_collection_names(None).await?;
...
pub async fn load_data(
    ...
    name: &str,
    records: &[Record],
) -> IgnoreError<InsertManyResult> {
    println!("Total records: {:#?}", records.len());
    Ok(db
        .collection::<Record>(name)
        .insert_many(records.iter(), None)
        .await?)
}
...
async fn main() -> IgnoreError {
    ...
    let db_connection = tokio::task::spawn(async move {
        let client = connect_to_mongodb(&opts.mongo_host, opts.mongo_port)
            .await
            .unwrap();
        let db = client.database(&collection_name);
        info!(
            "Established a connection to Mongodb at: {:#?}",
            client.list_databases(None, None).await.unwrap()
        );
    ...
    let features = std::thread::spawn(move || parse_geojson(opts.input).unwrap());

    let db = db_connection.await?;
    let features = features.join().unwrap();

    info!(
        "Finished loading geojson data in memory. Number of records: {:?}",
        features.len()
    );
